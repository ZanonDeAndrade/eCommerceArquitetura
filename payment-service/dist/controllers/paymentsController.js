...
const METODOS_PERMITIDOS = ["PIX", "Boleto", "Cartão"];
...
export const listarMetodosPagamento = (_req, res) => {
...
export const confirmarPagamento = async (req, res) => {
    try {
        ...
        const creationPromises = payments.map((payment) => prisma.payment.create({
            data: {
                orderId,
                userId: orderUserId,
        ...
        if (customerName) {
            ...
            }).catch((err) => console.warn("Falha ao publicar evento de pagamento confirmado:", err?.message ?? err));
        }
        res.status(200).json({
...
export const buscarPagamentosDoPedido = async (req, res) => {
    try {
